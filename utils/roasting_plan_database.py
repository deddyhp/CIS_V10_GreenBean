...
def save_new_plan(metadata: dict[str, Any], plan_data: dict[str, Any]) -> str:
    """Create a brand-new plan without any edit/update branch."""
    ws = _worksheet()
    now = _now()
    new_plan_id = _generate_id()
    source = metadata.get("source") or "Manual Plan"
    row = [
        new_plan_id,
        metadata.get("plan_name", ""), metadata.get("bean", ""),
        ...
        metadata.get("status", "Draft"), float(metadata.get("batch_size_g") or 0),
        float(metadata.get("density") or 0), float(metadata.get("moisture") or 0),
        json.dumps(plan_data, ensure_ascii=False), 0, 0,
        "", "", "", now, now, source,
    ]
    ws.append_row(row, value_input_option="USER_ENTERED")
    return new_plan_id
