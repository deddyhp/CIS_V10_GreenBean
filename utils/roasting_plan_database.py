...
def save_plan(metadata: dict[str, Any], plan_data: dict[str, Any], plan_id: str | None = None) -> str:
    ...
    row = [
        ...
        last_used_at, result_notes, cupping_notes, created_at, now,
        metadata.get("source", existing.get("source", "Manual Plan") if plan_id else "Manual Plan"),
    ]
    if _find_row(plan_id):
