...
def read_mobile_roast_upload(uploaded) -> tuple[str, bytes]:
    """Read .alog directly or extract the first .alog from a ZIP.

    Android file pickers sometimes do not recognize the .alog extension.
    Therefore uploaders use type=None and CIS validates the content itself.
    """
    if uploaded is None:
        raise ValueError("Belum ada file yang dipilih.")

    name = uploaded.name or "uploaded_file"
    payload = uploaded.getvalue()

    if not payload:
        raise ValueError("File kosong atau gagal dibaca dari perangkat.")
        ...
        try:
            with zipfile.ZipFile(BytesIO(payload)) as zf:
                alog_names = [
                    ...
                    if not item.is_dir() and item.filename.lower().endswith(".alog")
                ]
                if not alog_names:
                    raise ValueError("ZIP tidak berisi file Artisan .alog.")
                selected = alog_names[0]
                return Path(selected).name, zf.read(selected)
        except zipfile.BadZipFile as exc:
            raise ValueError("File ZIP tidak valid.") from exc

    if lower_name.endswith(".alog") or lower_name.endswith(".txt"):
        return name, payload

    # Some Android file managers may rename or hide the extension.
    # Accept content that looks like an Artisan dictionary.
    stripped = payload.lstrip()
    if stripped.startswith(b"{"):
        return name if "." in name else f"{name}.alog", payload

    raise ValueError(
        "Pilih file .alog, file .alog yang dibungkus ZIP, atau .txt berisi data Artisan."
    )

def _metric_value(value: Any, digits: int = 1) -> str:
    if value is None:
        return "—"
    try:
...
def _phase_metrics(parsed: ParsedRoast) -> dict[str, Any]:
    s = parsed.summary
    total = s.get("total_time") or s.get("drop_time")
    drying = s.get("drying_time")
    if drying is None:
        drying = s.get("dry_time")
    maillard = s.get("maillard_time")
    if maillard is None and s.get("fc_time") is not None and s.get("dry_time") is not None:
        maillard = s.get("fc_time") - s.get("dry_time")
    development = s.get("development_time")
    if development is None and s.get("drop_time") is not None and s.get("fc_time") is not None:
        development = s.get("drop_time") - s.get("fc_time")
    dtr = s.get("dtr_pct")
    if dtr is None and total and development is not None:
        dtr = development / total * 100
    return {
        "total_time": total,
        "drying_time": drying,
        "maillard_time": maillard,
        "development_time": development,
        "dtr_pct": dtr,
        "charge_bt": s.get("charge_bt"),
        "dry_bt": s.get("dry_bt"),
        "fc_bt": s.get("fc_bt"),
        "drop_bt": s.get("drop_bt"),
        "weight_loss_pct": s.get("weight_loss_pct"),
        "yield_pct": s.get("yield_pct"),
    }


def _diagnose_single_roast(parsed: ParsedRoast, purpose: str, cup_notes: str) -> list[tuple[str, str]]:
    m = _phase_metrics(parsed)
    findings: list[tuple[str, str]] = []

    total = m["total_time"]
    drying = m["drying_time"]
    maillard = m["maillard_time"]
    development = m["development_time"]
    dtr = m["dtr_pct"]
    loss = m["weight_loss_pct"]
    drop_bt = m["drop_bt"]

    if drying is not None:
        if drying < 230:
            findings.append(("Drying", "Cenderung cepat. Periksa risiko permukaan matang lebih dulu daripada bagian dalam bean."))
        elif drying > 330:
            findings.append(("Drying", "Cenderung panjang. Periksa potensi rasa flat, baked, atau energi roast yang kurang."))
        else:
            findings.append(("Drying", "Berada pada rentang kerja yang wajar untuk batch kecil."))

    if maillard is not None:
        if maillard < 150:
            findings.append(("Maillard", "Relatif singkat. Sweetness dan pembentukan body mungkin belum maksimal."))
        elif maillard > 300:
            findings.append(("Maillard", "Relatif panjang. Bisa meningkatkan body, tetapi perlu diawasi agar aroma tidak tertahan."))
        else:
            findings.append(("Maillard", "Proporsinya cukup seimbang untuk pengembangan sweetness dan body."))

    if dtr is not None:
        if purpose in {"Filter", "Sample Roast"}:
            if dtr < 12:
                msg = "Sangat singkat untuk filter; cek rasa grassy, sharp, atau underdeveloped."
            elif dtr > 20:
                msg = "Cukup panjang untuk filter; cek apakah acidity dan aroma tertutup."
            else:
                msg = "Masuk rentang umum filter; tetap validasi dengan cupping."
        elif purpose in {"Latte", "Espresso", "Tubruk"}:
            if dtr < 17:
                msg = "Cenderung pendek untuk profil berbasis susu/espresso; body dan solubility bisa kurang."
            elif dtr > 30:
                msg = "Sangat panjang; cek risiko roastiness, dryness, dan aroma yang menurun."
            else:
                msg = "Masuk rentang kerja yang masuk akal untuk profil lebih developed."
        else:
            if dtr < 14:
                msg = "Development relatif pendek; cek kematangan internal."
            elif dtr > 26:
                msg = "Development relatif panjang; cek roastiness dan kehilangan karakter origin."
            else:
                msg = "Development proporsional; arah akhir ditentukan oleh target rasa."
        findings.append(("Development", f"DTR {_metric_value(dtr)}%. {msg}"))

    if loss is not None:
        if loss < 13:
            msg = "Weight loss rendah; profil mungkin masih ringan atau kurang berkembang."
        elif loss > 19:
            msg = "Weight loss tinggi; cek risiko roast terlalu gelap atau terlalu lama."
        else:
            msg = "Weight loss berada dalam rentang yang lazim."
        findings.append(("Physical Result", f"Weight loss {_metric_value(loss)}%. {msg}"))

    if drop_bt is not None:
        findings.append(("Drop", f"Drop BT {_metric_value(drop_bt)}°C. Interpretasi akhir harus mengikuti kalibrasi probe dan target produk."))

    if parsed.events.empty:
        findings.append(("Machine Events", "Event gas/airflow tidak terbaca dari file; diagnosis energi roast masih terbatas."))
    else:
        gas_events = parsed.events["gas"].dropna() if "gas" in parsed.events else pd.Series(dtype=float)
        air_events = parsed.events["airflow"].dropna() if "airflow" in parsed.events else pd.Series(dtype=float)
        findings.append((
            "Machine Events",
            f"Terdeteksi {len(gas_events)} perubahan gas dan {len(air_events)} perubahan airflow dari Artisan."
        ))

    if cup_notes.strip():
        findings.append(("Cup Context", f"Catatan rasa pengguna: {cup_notes.strip()}"))

    if total is not None and development is not None:
        findings.append((
            "Priority Check",
            "Validasi hubungan antara bentuk RoR menjelang FC, momentum setelah FC, dan hasil cup—bukan hanya mengejar angka DTR."
        ))

    return findings


def _comparison_table(parsed_a: ParsedRoast, parsed_b: ParsedRoast, label_a: str, label_b: str) -> pd.DataFrame:
    a = _phase_metrics(parsed_a)
    b = _phase_metrics(parsed_b)
    rows = [
        ("Charge BT (°C)", a["charge_bt"], b["charge_bt"]),
        ("Dry End", fmt_time(a["drying_time"]), fmt_time(b["drying_time"])),
        ("Dry End BT (°C)", a["dry_bt"], b["dry_bt"]),
        ("Maillard", fmt_time(a["maillard_time"]), fmt_time(b["maillard_time"])),
        ("First Crack BT (°C)", a["fc_bt"], b["fc_bt"]),
        ("Development", fmt_time(a["development_time"]), fmt_time(b["development_time"])),
        ("DTR (%)", a["dtr_pct"], b["dtr_pct"]),
        ("Total Time", fmt_time(a["total_time"]), fmt_time(b["total_time"])),
        ("Drop BT (°C)", a["drop_bt"], b["drop_bt"]),
        ("Weight Loss (%)", a["weight_loss_pct"], b["weight_loss_pct"]),
    ]
    return pd.DataFrame(rows, columns=["Parameter", label_a, label_b])


def _render_comparison_chart(parsed_a: ParsedRoast, parsed_b: ParsedRoast, label_a: str, label_b: str) -> None:
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=parsed_a.curves["time_s"], y=parsed_a.curves["bt"],
        name=f"{label_a} — BT", mode="lines"
    ))
    fig.add_trace(go.Scatter(
        x=parsed_b.curves["time_s"], y=parsed_b.curves["bt"],
        name=f"{label_b} — BT", mode="lines"
    ))
    fig.update_layout(
        height=500,
...
def render_ai_roast_analysis() -> None:
    st.subheader("AI Roast Analysis")
    st.caption(
        "Rule-based roast diagnostic engine for single profile review and two-profile comparison. "
        "Works in Full Mode and Fallback Mode without saving to the main database."
    )

    mode = st.radio(
        "Analysis Type",
        ["Analyze Single Roast", "Compare Two Roasts"],
        horizontal=True,
    )

    purpose_options = [
        "Filter", "Espresso", "Latte", "Tubruk",
        "Blend Component", "Sample Roast", "Experimental", "Other"
    ]

    if mode == "Analyze Single Roast":
        uploaded = st.file_uploader(
            "Upload one Roast — ZIP atau TXT",
            type=["zip", "txt"],
            key="ai_single",
            help="Untuk tablet/HP: bungkus .alog menjadi ZIP atau rename salinan menjadi .txt."
        )
        purpose = st.selectbox("Roast target", purpose_options, key="ai_single_purpose")
        cup_notes = st.text_area(
            "Cup notes / result (optional)",
            placeholder="Example: sweet, bold, aroma good, slightly dry aftertaste...",
            key="ai_single_notes",
        )
        process_ai_single = st.button(
            "Process Single Roast",
            type="primary",
            key="ai_single_process",
            width="stretch",
        )
        if not process_ai_single:
            st.info("Pilih file, lalu tekan **Process Single Roast**.")
            return
        if uploaded is None:
            st.error("Belum ada file yang dipilih.")
            return
        try:
            file_name, payload = read_mobile_roast_upload(uploaded)
            st.success(f"File terbaca: {file_name} ({len(payload) / 1024:.1f} KB)")
            parsed = parse_alog(file_name, payload)
        ...
        s = parsed.summary
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Total Time", fmt_time(_phase_metrics(parsed)["total_time"]))
        c2.metric("DTR", f"{_metric_value(_phase_metrics(parsed)['dtr_pct'])}%")
        c3.metric("Drop BT", f"{_metric_value(s.get('drop_bt'))}°C")
        c4.metric("Weight Loss", f"{_metric_value(s.get('weight_loss_pct'))}%")

        st.plotly_chart(roast_chart(parsed), width="stretch")

        st.markdown("#### Diagnostic Findings")
        for title, message in _diagnose_single_roast(parsed, purpose, cup_notes):
            st.markdown(f"**{title}:** {message}")

        st.warning(
            "This analysis is a technical screening aid. Final judgment must use cupping results, "
            "machine behavior, bean properties, and the intended beverage."
        )
        return
    ...
    with left:
        upload_a = st.file_uploader(
            "Upload Roast A — ZIP atau TXT",
            type=["zip", "txt"],
            key="ai_compare_a"
        )
        label_a = st.text_input("Label A", value="Roast A", key="ai_label_a")
        purpose_a = st.selectbox("Target A", purpose_options, key="ai_purpose_a")
        notes_a = st.text_area("Cup notes A (optional)", key="ai_notes_a")
    with right:
        upload_b = st.file_uploader(
            "Upload Roast B — ZIP atau TXT",
            type=["zip", "txt"],
            key="ai_compare_b"
        )
        label_b = st.text_input("Label B", value="Roast B", key="ai_label_b")
        purpose_b = st.selectbox("Target B", purpose_options, key="ai_purpose_b")
        notes_b = st.text_area("Cup notes B (optional)", key="ai_notes_b")

    process_compare = st.button(
        "Process Two Roasts",
        type="primary",
        key="ai_compare_process",
        width="stretch",
    )

    if not process_compare:
        st.info("Pilih Roast A dan Roast B, lalu tekan **Process Two Roasts**.")
        return

    if upload_a is None or upload_b is None:
        st.error("Dua file belum lengkap dipilih.")
        return

    try:
        file_name_a, payload_a = read_mobile_roast_upload(upload_a)
        file_name_b, payload_b = read_mobile_roast_upload(upload_b)
        st.success(
            f"File terbaca: {file_name_a} dan {file_name_b}"
        )
        parsed_a = parse_alog(file_name_a, payload_a)
        parsed_b = parse_alog(file_name_b, payload_b)
    except ValueError as exc:
        st.error(str(exc))
        return

    _render_comparison_chart(parsed_a, parsed_b, label_a, label_b)

    st.markdown("#### Side-by-Side Metrics")
    st.dataframe(
        _comparison_table(parsed_a, parsed_b, label_a, label_b),
        hide_index=True,
        width="stretch",
    )

    st.markdown(f"#### {label_a} — Diagnostic")
    for title, message in _diagnose_single_roast(parsed_a, purpose_a, notes_a):
        st.markdown(f"**{title}:** {message}")

    st.markdown(f"#### {label_b} — Diagnostic")
    for title, message in _diagnose_single_roast(parsed_b, purpose_b, notes_b):
        st.markdown(f"**{title}:** {message}")

    ma = _phase_metrics(parsed_a)
    mb = _phase_metrics(parsed_b)
    st.markdown("#### Comparison Focus")
    comparison_notes = []

    if ma["drying_time"] is not None and mb["drying_time"] is not None:
        delta = ma["drying_time"] - mb["drying_time"]
        comparison_notes.append(
            f"Drying difference: {abs(delta):.0f} seconds; "
            f"{label_a if delta > 0 else label_b} is longer."
        )
    if ma["development_time"] is not None and mb["development_time"] is not None:
        delta = ma["development_time"] - mb["development_time"]
        comparison_notes.append(
            f"Development difference: {abs(delta):.0f} seconds; "
            f"{label_a if delta > 0 else label_b} is longer."
        )
    if ma["dtr_pct"] is not None and mb["dtr_pct"] is not None:
        delta = ma["dtr_pct"] - mb["dtr_pct"]
        comparison_notes.append(
            f"DTR difference: {abs(delta):.1f} points; "
            f"{label_a if delta > 0 else label_b} is higher."
        )
    if ma["weight_loss_pct"] is not None and mb["weight_loss_pct"] is not None:
        delta = ma["weight_loss_pct"] - mb["weight_loss_pct"]
        comparison_notes.append(
            f"Weight-loss difference: {abs(delta):.1f} points; "
            f"{label_a if delta > 0 else label_b} is higher."
        )
    ...
    if comparison_notes:
        ...
        st.info("Not enough shared metrics for a numerical comparison.")

    st.success(
        "Use the cup notes to decide which curve behavior should become the reference. "
        "A longer or hotter profile is not automatically better."
    )
...
def app() -> None:
    ...
    st.caption("CIS Roast Log V0.4.9 — Android ZIP/TXT upload mode and AI Roast Analysis.")
    ...
    if FULL_MODE:
        ...
        st.info(
            ...
            "Preview mobile memakai ZIP atau TXT karena Android sering tidak mengirim file .alog langsung. "
            "Penyimpanan permanen, Bulk Import, dan Roast Database utama hanya tersedia dari laptop / Remote."
        )

    page = st.selectbox(
        "Roast Profile Menu",
        ["Home", "Import Roast", "Bulk Import", "Roast Database", "AI Roast Analysis"],
        key="roast_profile_main_menu",
    )
    if page == "AI Roast Analysis":
        render_ai_roast_analysis()
        return
    ...
    if page == "Home":
        if FULL_MODE:
            ...
            st.write(
                "Gunakan **Import Roast** untuk membuka satu roast melalui **ZIP atau TXT** "
    ...
    uploaded = st.file_uploader(
        "Upload Roast File — ZIP atau TXT",
        type=["zip", "txt"],
        key="daily_roast_upload",
        help="Untuk tablet/HP: bungkus .alog menjadi ZIP atau rename salinan menjadi .txt.",
    )
    ...
    process_upload = st.button(
        "Process Upload",
        type="primary",
        key="daily_process_upload",
        width="stretch",
    )

    if not process_upload:
        st.info("Pilih file terlebih dahulu, lalu tekan **Process Upload**.")
        return

    if uploaded is None:
        st.error("Belum ada file yang dipilih. Pilih file lalu tekan Process Upload.")
        return
    ...
    try:
        ...
        st.error(f"File gagal diproses: {exc}")
        return
    except Exception as exc:
        st.error(f"Terjadi kesalahan saat membaca file: {exc}")
        return

    # Duplicate checking is only valid when the permanent local database is active.
    duplicate = existing_checksum(parsed.checksum) if FULL_MODE else None
    if duplicate:
