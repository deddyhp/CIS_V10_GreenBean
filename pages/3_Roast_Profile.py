...
def render_ai_roast_analysis() -> None:
    ...
    if mode == "Analyze Single Roast":
        uploaded = st.file_uploader("Upload one Artisan .alog", type=["alog"], key="ai_single")
    ...
    left, right = st.columns(2)
    with left:
        upload_a = st.file_uploader("Upload Roast A", type=["alog"], key="ai_compare_a")
    ...
    with right:
        upload_b = st.file_uploader("Upload Roast B", type=["alog"], key="ai_compare_b")
    ...
    try:
        parsed_a = parse_alog(upload_a.name, upload_a.getvalue())
        parsed_b = parse_alog(upload_b.name, upload_b.getvalue())
    except ValueError as exc:
        st.error(str(exc))
        return
...
def app() -> None:
    ...
    st.caption("CIS Roast Log V0.4.6 — mobile upload fix, database review, and AI Roast Analysis.")
    ...
    st.success(f"File terpilih: {uploaded.name} ({uploaded.size / 1024:.1f} KB)")

    if not uploaded.name.lower().endswith(".alog"):
        st.error("File harus berformat Artisan .alog.")
        return

    payload = uploaded.getvalue()
    if not payload:
        st.error("File kosong atau gagal dibaca dari perangkat.")
        return

    try:
        parsed = parse_alog(uploaded.name, payload)
    except ValueError as exc:
