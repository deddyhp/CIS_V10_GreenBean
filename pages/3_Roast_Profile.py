...
def render_ai_roast_analysis() -> None:
    ...
    if mode == "Analyze Single Roast":
        ...
        if not uploaded:
            st.info("Upload one .alog file to begin the analysis.")
            return
        try:
    ...
    if not upload_a or not upload_b:
        st.info("Upload two .alog files to compare.")
        return
...
def app() -> None:
    ...
    st.caption("CIS Roast Log V0.4.7 — Android upload compatibility, ZIP support, and AI Roast Analysis.")
    ...
    uploaded = st.file_uploader(
        ...
        help="Pada beberapa tablet/HP, ekstensi .alog tidak dikenali oleh file picker. "
             "Karena itu semua file ditampilkan, lalu CIS memvalidasi file setelah dipilih.",
    )
    if not uploaded:
        st.info("Pilih satu file Artisan .alog untuk mulai.")
        return
